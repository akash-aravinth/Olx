...
public class SellDao {
    ...
    private String addQuery = "insert into selltable(userid,productid) values(?,?)";
    private String getSell = "select productid from selltable where userid = ?";
    public void addSellProduct(int userId, int productId) {
        try{
    ...
    public List<Product> getSellList(int userId){
        List<Integer> orders = new ArrayList<>();
        List<Product> products = new ArrayList<>();
        try{
            connection = Database.getInstance().getConnection();
            PreparedStatement statement = connection.prepareStatement(getSell);
            statement.setInt(1,userId);
